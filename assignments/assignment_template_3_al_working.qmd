...
```{r}
data_values <- readr::read_csv("data/values_data.csv") # Finish the code
```
...
data_values <- 
  data_values |>
  dplyr::mutate(
    Q46_binary = dplyr::if_else(Q46 %in% 3:4, 0, 1), # Finish code
    Q47_binary = dplyr::if_else(Q47 %in% 3:5, 0, 1) # Finish code
  )
```
...
```{r}
mean( data_values$Q46_binary, na.rm = TRUE) # Finish the code
mean( data_values$Q47_binary, na.rm = TRUE) # Finish the code
```
...
```{r}
total_happy <- sum(data_values$Q46_binary , na.rm = TRUE) # Finish the code
total_responses <- sum(!is.na(data_values$Q46_binary)) # Total non-missing responses

prop.test(total_happy, total_responses , p = .50, alternative = "two.sided", correct = FALSE) # Finish the code
```
...
data_values_us_can <- 
  data_values |>
  dplyr::filter(
    B_COUNTRY_ALPHA %in% c("USA", "CAN") # Finish code here
  )
```
...
data_usa_can_summ <- 
  ...
  dplyr::summarize(
    ...
    .by = B_COUNTRY_ALPHA # Finish code
  )
```
...
```{r}
prop.test(data_usa_can_summ$count_wellbeing_responses, data_usa_can_summ$total_responses, alternative = "two.sided", correct = FALSE)
```
...
data_usa_can_summ <- 
  data_values_us_can |>
  dplyr::summarize(
    count_health_responses = sum(Q47_binary, na.rm = TRUE),
    total_responses = sum(!is.na(Q46_binary)),
    .by = B_COUNTRY_ALPHA # Finish code
  )

prop.test(data_usa_can_summ$count_health_responses, data_usa_can_summ$total_responses, alternative = "two.sided", correct = FALSE)
```
