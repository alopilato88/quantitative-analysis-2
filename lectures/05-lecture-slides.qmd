...
We can compare the fit of nested models using a **deviance test** which compares the log likelhood of the smaller (less complicated) model to the larger (more complicated) model. 

A signficant **deviance test** means the more complicated model fits our data better than the less complicated model.
...
The predicted outcome threshold (.50 in our example) is arbritrary. It is possible that a different choice of threshold would give us a better true positive and true negative rate. 
