...
data_predict <- 
  tibble::tibble(
    AGE_STD = 0, 
    Q287_SES = median(data_analysis$Q287_SES, na.rm = TRUE), 
    Q288_INCOME = median(data_analysis$Q288_INCOME, na.rm = TRUE),
    Q260_SEX = "F",
...
data_predict <- 
  data_predict |>
  dplyr::mutate(
    pred_prob = predict(mod_interact, data_predict, type = "response")
  )

ggplot2::ggplot(
  data_predict, 
  ggplot2::aes(
     x = Q252_SAT_POL_SYSTEM,
     y = pred_prob
  )
) + 
  ggplot2::facet_wrap(~Q216_ENCOURAGE_VOTE) + 
  ggplot2::geom_point()
```
...
mod_nonlinear <- glm(
  VOTE_NATIONAL ~ AGE_STD + AGE_STD_SQ +  Q287_SES + Q288_INCOME + Q216_ENCOURAGE_VOTE  +
    Q252_SAT_POL_SYSTEM + Q260_SEX, 
  data = data_analysis, 
  family = binomial(link = "logit")
)
...
data_predict <- 
  tibble::tibble(
    ...
    Q288_INCOME = median(data_analysis$Q288_INCOME, na.rm = TRUE),
    Q260_SEX = "F",
    Q216_ENCOURAGE_VOTE = "Might Do", 
    Q252_SAT_POL_SYSTEM = median(data_analysis$Q252_SAT_POL_SYSTEM, na.rm = TRUE)
  ) |>
  dplyr::mutate(
    AGE_STD_SQ = AGE_STD * AGE_STD
  )
...
data_predict <- 
  data_predict |>
  dplyr::mutate(
    pred_prob = predict(mod_nonlinear, data_predict, type = "response")
  )
