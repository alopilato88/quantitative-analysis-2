...
```{r}
mod_age <- glm(Q222_VOTE_NATIONAL ~ AGE_STD, 
           data = data_analysis |> 
             dplyr::mutate(Q222_VOTE_NATIONAL = dplyr::if_else(Q222_VOTE_NATIONAL == "Yes", 1, 0)),
           family = binomial
           )
```
...
Using the results of your model above, provide an interpretation of the effect of `AGE_STD` on `Q222_VOTE_NATIONAL`. Be sure to note if the effect is significant and provide an interpretation of the coefficient.
...
data_predict <- 
  data_predict |>
  dplyr::mutate(
    pred_prob = predict( mod_income, data_predict, type = "response")
  )
...
```{r}
mod_encourage <- glm(Q222_VOTE_NATIONAL ~ Q216_ENCOURAGE_VOTE, 
           data = data_analysis |> 
             dplyr::mutate(Q222_VOTE_NATIONAL = dplyr::if_else(Q222_VOTE_NATIONAL == "Yes", 1, 0)),
           family = binomial
           )
...
Fit univareitat with categorical interpret
